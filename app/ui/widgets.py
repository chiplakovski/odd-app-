...
class ActivityRow(QFrame):
    """A row in the Work Order History list: a kind badge, title/subtitle, and
    Open/Delete actions (mirrors ReportFileRow's stacked-vertical button layout)."""
    ...
    def __init__(self, kind_label: str, kind_color: str, title: str, subtitle: str) -> None:
        ...
        # Minimum, not fixed, width - see ReportFileRow._wrappable's neighboring
        # comment: a fixed budget sized against this sandbox's font metrics still
        # clipped button text under Windows' actual font rendering.
        button_layout = QVBoxLayout()
        button_layout.setSpacing(4)
        open_button = QPushButton("Open")
        open_button.setObjectName("secondaryButton")
